...
**狀態**：架構定案（2026-07-18）・整合層 v0 開發中
**同仁說明網頁**：<https://med95albert.github.io/clinic-photo-archive/>

## 這個 repo 是什麼

這是一個**架構與文件 repo**（尚不含可執行系統，整合層程式碼將於 v0 完成後併入）：
...
本 repo 為純架構文件：不含任何病人資料、不含診所內部帳號或金鑰。文件中的身分證字號一律以後四碼遮罩的格式示意（如 `A12345****`）呈現，不出現任何完整且通過檢查碼的證號。
