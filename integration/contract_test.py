...
from clinic_archive.redact import mask_segment  # noqa: E402
...
def _fmt_names(paths: list[Path], limit: int = 8) -> str:
    # 輸出會被現場 agent 讀進對話：證號遮罩、非系統產生的檔名換雜湊（可能含姓名）。
    if not paths:
        return "（無）"
    names = [mask_segment(p.name) for p in paths[:limit]]
...
def _check_sequence_contiguous(walk: _Walk) -> ClauseResult:
    ...
    for dir_path, files in walk.files_by_dir.items():
        for (date, tm), items in sorted(_group_by_batch(files).items()):
            ...
            if idxs != expected:
                bad_groups.append(
                    f"{mask_segment(dir_path.name)}/{date}_{tm}（{len(items)} 個檔案）"
...
def _check_top_level_layout(walk: _Walk) -> ClauseResult:
    ...
    for dir_path, subdirs in walk.nested_subdirs.items():
        problems.append(f"{mask_segment(dir_path.name)}/ 底下有巢狀子資料夾（應只有一層）：{_fmt_names(subdirs)}")
    for dir_path in walk.top_dirs:
        if not dir_path.name.strip():
            problems.append("發現名稱為空白的第一層資料夾")

    passed = not problems
    names = [mask_segment(d.name) for d in walk.top_dirs]  # 淨化：輸出會進 agent 對話
